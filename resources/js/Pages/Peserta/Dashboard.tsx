...
function getCicilanSummary(pendaftar: Pendaftar) {
  ...
  const totalBiaya = pendaftar.biaya_pelatihan || 0;
...
export default function PesertaDashboardPage() {
  ...
  const [kelulusanRecord, setKelulusanRecord] = useState<Kelulusan | null>(null);
  ...
  const totalHargaFormatted = program?.harga ? `Rp ${program.harga.toLocaleString('id-ID')}` : `Rp ${(pendaftar.biaya_pelatihan || 3500000).toLocaleString('id-ID')}`;

  const filteredJadwal = jadwalList.filter(
    (j) => j.jenis_pelatihan === 'Semua' || j.jenis_pelatihan === pendaftar.jenis_pelatihan || j.jenis_pelatihan === program?.nama
  );

  const pretestHasil = hasilUjianList.find((h) => h.tipe === 'pretest');
  const posttestHasil = hasilUjianList.find((h) => h.tipe === 'posttest');
    ...
    <>
      ...
      <main className="flex-1 py-8 md:py-12">
        <div className="container-wide">
          {/* Header Banner */}
          <div className="glass-card p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <span className="badge badge-accepted">Pendaftaran Diterima</span>
                <span className={getStatusPembayaranBadgeClass(pendaftar.status_pembayaran)}>
                  {getStatusPembayaranLabel(pendaftar.status_pembayaran)}
                </span>
              </div>
              <h1 className="text-2xl md:text-3xl font-extrabold text-[var(--text-primary)]">
                Selamat Datang, {pendaftar.nama_lengkap}!
              </h1>
              <p className="text-xs md:text-sm text-[var(--text-secondary)] mt-1 font-mono">
                No. Pendaftaran: <span className="font-bold text-[var(--primary)]">{pendaftar.no_pendaftaran}</span> • Username: {pendaftar.user?.email || '-'}
              </p>
            </div>
            <button onClick={handleLogout} className="btn btn-outline btn-sm self-start md:self-auto">
          ...
          <div className="dashboard-grid">
            {/* Sidebar Navigation Peserta */}
            <aside className="sidebar-nav h-fit space-y-1">
              <div className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-[var(--text-tertiary)] mb-1">
                Portal Peserta
              </div>
              ...
              {[
                {
                  ...
                  badge: pendaftar.status_pembayaran === 'lunas' ? 'LUNAS' : 'ACTION',
                  badgeColor: pendaftar.status_pembayaran === 'lunas' ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-white',
                },
                {
                  id: 'profile_peserta',
                ...
                },
                {
                  ...
                  badge: filteredJadwal.length,
                  badgeColor: 'bg-[var(--primary-bg)] text-[var(--primary)]',
                },
                {
                  id: 'ujian',
                  ...
                  badge: pretestHasil && posttestHasil ? 'SELESAI' : 'UJIAN',
                  badgeColor: pretestHasil && posttestHasil ? 'bg-emerald-600 text-white' : 'bg-indigo-600 text-white',
                },
                {
                  id: 'rincian_program',
                ...
                },
                {
                  ...
                  badge: kelulusanRecord?.status_kelulusan === 'Lulus' ? 'SERTIFIKAT' : null,
                  badgeColor: 'bg-amber-500 text-white',
                },
              ].map((item) => (
                <button
                  key={item.id}
                  onClick={() => setActiveTab(item.id as PesertaTab)}
                ...
                <div className="glass-card-static p-6 md:p-8 animate-fade-in space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      ...
                      <ProfileItem label="Jenis Kelamin" value={pendaftar.jenis_kelamin} />
                      <ProfileItem label="Tempat, Tanggal Lahir" value={`${pendaftar.tempat_lahir}, ${new Date(pendaftar.tanggal_lahir).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}`} />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      ...
                      <ProfileItem label="Kontak Darurat" value={`${pendaftar.nama_kontak_darurat} (${pendaftar.hubungan_kontak_darurat})`} />
                      <ProfileItem label="No. HP Darurat" value={pendaftar.no_hp_kontak_darurat} />
                    </div>
                  </div>
                </div>
              )}
                    ...
                    <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl self-start sm:self-auto">
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${
                          jadwalViewMode === 'calendar' ? 'bg-white text-[var(--primary)] shadow-sm' : 'text-slate-500 hover:text-slate-800'
                        }`}
                      >
                        Tampilan Kalender
                      </button>
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${
                          jadwalViewMode === 'list' ? 'bg-white text-[var(--primary)] shadow-sm' : 'text-slate-500 hover:text-slate-800'
                        }`}
                      >
                        Tampilan List
                      </button>
                ...
                <div className="glass-card-static p-6 md:p-8 animate-fade-in space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="p-6 rounded-2xl bg-[var(--surface)] border border-[var(--card-border)] flex flex-col justify-between space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className={`text-xs font-bold ${pretestHasil ? 'text-emerald-600' : 'text-amber-600'}`}>
                            {pretestHasil ? 'Sudah Dikerjakan' : 'Belum Dikerjakan'}
                          </span>
                        </div>
                      ...
                      {pretestHasil ? (
                        <div className="p-4 rounded-xl bg-white border border-[var(--card-border)] space-y-1">
                          <span className="text-[10px] uppercase font-bold text-[var(--text-tertiary)]">Hasil Ujian Pretest Anda:</span>
                    ...
                    <div className="p-6 rounded-2xl bg-[var(--surface)] border border-[var(--card-border)] flex flex-col justify-between space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className={`text-xs font-bold ${posttestHasil ? 'text-emerald-600' : 'text-amber-600'}`}>
                            {posttestHasil ? 'Sudah Dikerjakan' : 'Belum Dikerjakan'}
                          </span>
                        </div>
                      ...
                      {posttestHasil ? (
                        <div className="p-4 rounded-xl bg-white border border-[var(--card-border)] space-y-1">
                          <span className="text-[10px] uppercase font-bold text-[var(--text-tertiary)]">Hasil Ujian Posttest Anda:</span>
                        ...
                        <button
                          onClick={() => openExamModal('posttest')}
                          className="btn btn-accent btn-md w-full font-bold"
                        >
                ...
                {currentSoal?.opsi.map((opt, oIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left rounded-2xl border p-4 md:p-5 transition-all flex items-center gap-4 ${
                        isChosen
